            ...
            {infoMenu.map((el) => (
              <div className="accordion-item" key={el.id}>
                <h2 className="accordion-header">
                  <button className="accordion-button collapsed bg-light" type="button" data-bs-toggle="collapse" data-bs-target={`#flush-collapse${el.id}`} aria-expanded="false" aria-controls={`flush-collapse${el.id}`}>
                    <span className="material-symbols-outlined">{el.iconMaterial}</span> &nbsp; {el.category}
                  </button>
                </h2>
                <div
                  id={`flush-collapse${el.id}`} className="accordion-collapse collapse" data-bs-parent="#accordionFlushExample">
